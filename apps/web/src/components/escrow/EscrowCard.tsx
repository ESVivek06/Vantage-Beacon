  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between gap-3 mb-3">
        <div className="flex items-center gap-2.5 min-w-0">
          ...
          <div className="min-w-0">
            <p className="text-sm font-semibold text-neutral-900 truncate">{counterparty.name}</p>
            <p className="text-xs text-neutral-500">{counterparty.role}</p>
          </div>
        </div>
        <span className={cn('inline-flex items-center gap-1 rounded-full text-xs font-medium px-2 py-0.5 shrink-0', cfg.badgeClass)}>
      ...
      <p className="text-xs text-neutral-500 mb-3">
        {fmt(escrow.totalAmount)} ·{' '}
        {escrow.paymentStructure === 'milestone' ? `${totalMilestones} milestones` : 'Single payment'}
      </p>
      ...
      {escrow.paymentStructure === 'milestone' && totalMilestones > 0 && !isPending && (
        <div className="mb-3">
          <p className="text-xs text-neutral-500 mb-1">
            {nextPendingMilestone ? `Next: ${nextPendingMilestone.description}` : 'All milestones released'}
          </p>
          ...
          <p className="text-xs text-neutral-400 mt-1">{releasedMilestones}/{totalMilestones} milestones released</p>
        </div>
      )}
      ...
      <div className="flex flex-wrap gap-2 mt-4">
        {isDisputed ? (
          <>
            <Button variant="ghost" size="sm" onClick={() => onViewDetails(escrow.id)}>View details</Button>
            {escrow.dispute && (
          ...
          <>
            <Button variant="ghost" size="sm" onClick={() => onViewDetails(escrow.id)}>View full terms</Button>
            <Button variant="secondary" size="sm" onClick={() => onDecline?.(escrow.id)}>Decline</Button>
            <Button variant="success" size="sm" onClick={() => onAccept?.(escrow.id)}>Accept</Button>
          </>
        ) : (
          <>
            <Button variant="ghost" size="sm" onClick={() => onViewDetails(escrow.id)}>View details</Button>
            ...
            {viewerRole === 'counterparty' && escrow.status === 'IN_PROGRESS' && (
              <Button variant="secondary" size="sm" onClick={() => onRequestRelease?.(escrow.id)}>Request release</Button>
            )}
          </>
        )}
      </div>
