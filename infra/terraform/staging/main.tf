terraform {
  ...
  backend "s3" {
    bucket = "vb-tf-state-345307375390"
...
# ── DNS ──────────────────────────────────────────────────────────────────────
# NOTE: vb.com DNS zone is managed by Moniker (ns1-4.monikerdns.net).
# It is NOT in Route53 account 345307375390. DNS records must be added
# manually in the Moniker control panel (moniker.com) after ALB is created.
# Run scripts/add-staging-dns.sh to get the exact CNAME values to add.
